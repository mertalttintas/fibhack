...
export function Sidebar({
  ...
  return (
    <aside className="fixed left-0 top-0 h-full w-60 border-r border-white/[0.07] bg-navy-900/70 backdrop-blur-xl flex flex-col z-40">
      <div className="flex items-center gap-3 px-5 h-16 border-b border-white/[0.07]">
        <DiamondLogo size={30} />
        <div className="leading-tight">
          <div className="text-sm font-bold text-white tracking-tight">deneme</div>
      ...
      <nav className="flex-1 px-3 py-4 space-y-1">
        {items.map(({ key, label, icon: Icon }) => {
          ...
          return (
            ...
            >
              <Icon size={17} className={active ? "drop-shadow-[0_0_6px_rgba(47,182,166,0.8)]" : ""} />
              {label}
      ...
      <div className="px-5 py-4 border-t border-white/[0.07]">
        <div className="glass px-3 py-2.5 flex items-center gap-2.5">
          ...
          <div className="text-[11px] leading-tight">
            <div className="text-slate-300 font-medium">Orchestrator aktif</div>
