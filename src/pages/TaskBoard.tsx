...
function DepartmentTask({ task, onOpen, onPrepare }: { task: DeptTask; onOpen: () => void; onPrepare: () => void }) {
  ...
  return <motion.div layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} onClick={onOpen} className={cn("group w-full cursor-pointer rounded-xl border bg-[#09182b] p-3 text-left transition hover:border-white/[.15]", task.status === "processing" ? "border-fteal/30" : "border-white/[.065]")}><div className="mb-2 flex items-center justify-between"><span className="flex items-center gap-1 font-mono text-[7px] uppercase tracking-wider text-fteal"><Bot size={8} /> Team-bot talebi</span><span className="flex items-center gap-1.5">{task.teams && <MessagesSquare size={9} className={task.teams.delivered ? "text-fgreen-light" : "text-[#9EA2F0]"} />}{task.ai ? <span className="flex items-center gap-1 rounded-full border border-fteal/15 px-1.5 py-0.5 text-[8px] text-fteal-light"><Sparkles size={7} /> AI</span> : <span className="text-[8px] text-slate-700">DEMO</span>}</span></div><div className="rounded-lg border border-white/[.04] bg-white/[.018] px-2.5 py-2 text-[9px] leading-4 text-slate-500"><span className="text-slate-300">{task.department} ekibi,</span> yeni kampanya için sizden aşağıdaki çalışma bekleniyor.</div><div className="mt-2.5 text-[11px] font-medium leading-[17px] text-slate-200">{task.title}</div><div className="mt-1.5 line-clamp-2 text-[9px] leading-4 text-slate-600">{task.summary}</div>{task.details?.dataSources?.length ? <div className="mt-2 text-[8px] text-slate-700">Talep edilen veri: <span className="text-slate-500">{task.details.dataSources.slice(0,2).join(" · ")}</span></div> : null}<div className="mt-2.5 flex items-center justify-between border-t border-white/[.05] pt-2">{task.status === "waiting" ? <button onClick={(event) => { event.stopPropagation(); onPrepare(); }} className="flex items-center gap-1 rounded-md border border-fteal/25 bg-fteal/[.07] px-2 py-1 text-[8px] font-semibold text-fteal-light transition hover:bg-fteal/[.13]"><Sparkles size={8} /> AI'ya hazırlat</button> : task.status === "processing" ? <span className="flex items-center gap-1.5 text-[8px] text-fteal-light"><LoaderCircle size={9} className="animate-spin" /> AI taslak hazırlıyor…</span> : <span className="text-[8px]" style={{ color: dept.color }}>{task.status === "done" ? "Tamamlandı" : task.draft ? "Departmanda · AI taslağı hazır" : "Departmanda"}</span>}<ChevronRight size={10} className="text-slate-700 transition group-hover:text-fteal-light" /></div>{task.status === "processing" && <div className="mt-2 h-0.5 overflow-hidden rounded-full bg-white/[.05]"><motion.div className="h-full w-1/3 rounded-full bg-gradient-to-r from-transparent via-fteal to-transparent" animate={{ x: ["-100%", "300%"] }} transition={{ duration: 1.2, repeat: Infinity, ease: "linear" }} /></div>}</motion.div>;
}
...
function TaskDrawer({ task, onClose, onAdvance, onPrepare, onSendTeams, sendingTeams }: { task: DeptTask; onClose: () => void; onAdvance: () => void; onPrepare: () => void; onSendTeams: () => void; sendingTeams: boolean }) {
  ...
  return <><button onClick={onClose} className="fixed inset-0 z-40 bg-[#020914]/75 backdrop-blur-sm" /><motion.aside initial={{ x: 500 }} animate={{ x: 0 }} exit={{ x: 500 }} className="fixed bottom-0 right-0 top-0 z-50 w-[470px] overflow-y-auto border-l border-white/10 bg-[#071426]/[.98] p-6"><div className="mb-6 flex items-center justify-between"><div className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[.18em] text-fteal-light"><Bot size={13} /> Team-bot departman paketi</div><button onClick={onClose} className="grid h-8 w-8 place-items-center rounded-lg border border-white/10 text-slate-400"><X size={14} /></button></div><div className="mb-3 inline-flex rounded-md px-2 py-1 text-[9px] font-bold uppercase" style={{ color: dept.color, background: dept.soft }}>{task.department}</div><div className="mb-4 rounded-xl border border-fteal/12 bg-fteal/[.035] p-3 text-[11px] leading-5 text-slate-400"><span className="font-semibold text-slate-200">Merhaba {task.department} ekibi.</span><br />Onaylanan kampanya için aşağıdaki ihtiyaç ve geliştirme paketi size yönlendirildi.</div><h2 className="text-xl font-semibold leading-7 text-white">{task.title}</h2><p className="mt-2 text-sm leading-6 text-slate-400">{task.summary}</p>
    ...
    {task.draft && <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mt-5 rounded-2xl border border-fteal/20 bg-fteal/[.04] p-4">
      <div className="mb-2 flex items-center justify-between gap-3"><div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-fteal-light"><Sparkles size={12} /> AI ön çalışması</div><span className={cn("shrink-0 rounded-full border px-2 py-0.5 text-[8px]", task.draft.source === "ai" ? "border-fgreen/25 bg-fgreen/[.06] text-fgreen-light" : "border-white/[.12] text-slate-500")}>{task.draft.source === "ai" ? `${PROVIDER[task.draft.provider ?? ""] ?? task.draft.provider} · ${task.draft.model}` : "Şablon taslak"}</span></div>
      <div className="text-xs font-semibold leading-5 text-slate-200">{task.draft.headline}</div>
      <div className="mt-3 space-y-2">{task.draft.items.map((item) => <div key={item.title} className="rounded-xl border border-white/[.06] bg-[#061426]/70 p-3"><div className="text-[8px] font-semibold uppercase tracking-wider text-slate-500">{item.title}</div><div className="mt-1.5 text-[11px] leading-5 text-slate-300">{item.content}</div></div>)}</div>
    ...
    {task.status === "waiting" && <button onClick={onPrepare} className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-fteal to-fgreen py-3 text-sm font-bold text-[#061426]"><Sparkles size={14} /> AI ile hazırla ve departmana ata</button>}
    ...
    {task.status === "assigned" && <button onClick={onAdvance} className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-fteal to-fgreen py-3 text-sm font-bold text-[#061426]">Onayla ve ilerlet <ArrowRight size={14} /></button>}
    {task.teams ? (
      <div className={cn("mt-2.5 rounded-xl border p-3", task.teams.delivered ? "border-fgreen/25 bg-fgreen/[.05]" : "border-[#6264A7]/30 bg-[#6264A7]/[.08]")}>
        <div className={cn("flex items-center gap-2 text-xs font-semibold", task.teams.delivered ? "text-fgreen-light" : "text-[#B4B8F5]")}><MessagesSquare size={13} /> {task.teams.delivered ? "Teams kanalına iletildi" : "Teams gönderimi kaydedildi (demo modu)"} <span className="ml-auto font-mono text-[9px] opacity-70">{time(task.teams.sentAt)}</span></div>
        <div className="mt-1.5 text-[9px] leading-4 text-slate-500">{task.teams.delivered ? `Görev paketi Adaptive Card olarak ${task.teams.channel} kanalına düştü.` : `Webhook bağlanınca paket ${task.teams.channel} kanalına gerçek zamanlı düşecek. İçerik hazır ve kayıt altında.`}</div>
        <button onClick={onSendTeams} disabled={sendingTeams} className="mt-2 flex items-center gap-1.5 rounded-md border border-white/[.1] px-2 py-1 text-[9px] text-slate-400 transition hover:text-slate-200 disabled:opacity-40">{sendingTeams ? <LoaderCircle size={10} className="animate-spin" /> : <MessagesSquare size={10} />} Tekrar gönder</button>
      </div>
    ) : (
      <button onClick={onSendTeams} disabled={sendingTeams} className="mt-2.5 flex w-full items-center justify-center gap-2 rounded-xl border border-[#6264A7]/40 bg-[#6264A7]/[.14] py-2.5 text-xs font-semibold text-[#B4B8F5] transition hover:bg-[#6264A7]/[.22] disabled:opacity-60">{sendingTeams ? <><LoaderCircle size={13} className="animate-spin" /> Teams'e iletiliyor…</> : <><MessagesSquare size={13} /> {DEPT[task.department].teams} kanalına gönder</>}</button>
    )}
  </motion.aside></>;
}
