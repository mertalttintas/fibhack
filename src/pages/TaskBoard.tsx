...
import { localDraft, statusColumns, type CampaignJob, type DeptCard, type DeptDraft, type DeptTask, type TaskStatus, type TraceEvent } from "../data/mock";
...
function CampaignQueue({ campaigns, selectedId, onSelect, onNew }: { campaigns: CampaignJob[]; selectedId?: string; onSelect: (id: string) => void; onNew: () => void }) {
  return (
    <aside className="rounded-[22px] border border-[#0d2b45]/[.07] bg-[#ffffff]/85 p-3">
      ...
      <div className="mt-2 space-y-2">
        {campaigns.map((campaign) => {
          ...
          return <button key={campaign.id} onClick={() => onSelect(campaign.id)} className={cn("w-full rounded-xl border p-3 text-left transition", active ? "border-fteal/30 bg-fteal/[.065]" : "border-[#0d2b45]/[.055] bg-[#0d2b45]/[.018] hover:border-[#0d2b45]/[.12]")}><div className="mb-2 flex items-center justify-between"><span className="flex items-center gap-1.5 font-mono text-[8px] uppercase tracking-wider" style={{ color: state.color }}><span className={cn("h-1.5 w-1.5 rounded-full", campaign.status === "processing" && "animate-pulse")} style={{ background: state.color }} />{state.label}</span><span className="font-mono text-[8px] text-slate-400">{time(campaign.createdAt)}</span></div><div className="line-clamp-2 text-[11px] font-medium leading-[17px] text-slate-800">{campaign.title}</div><div className="mt-2 flex items-center justify-between gap-2">{campaign.provider ? <span className="flex items-center gap-1 text-[8px] text-slate-500"><Sparkles size={8} /> {PROVIDER[campaign.provider] ?? campaign.provider} · {campaign.model}</span> : <span />}{typeof campaign.score === "number" && <span className="shrink-0 rounded-md border border-fgreen/20 bg-fgreen/[.05] px-1.5 py-0.5 font-mono text-[8px] text-fgreen-light">skor {campaign.score}</span>}</div></button>;
        })}
        {campaigns.length === 0 && <div className="grid h-44 place-items-center rounded-xl border border-dashed border-[#0d2b45]/[.07] px-5 text-center"><div><Target className="mx-auto mb-2 text-slate-400" size={20} /><div className="text-[10px] leading-4 text-slate-500">Henüz kampanya yok.<br />Yeni bir fikir oluşturun.</div></div></div>}
      </div>
    </aside>
  );
}
...
function DepartmentTask({ task, onOpen, onPrepare }: { task: DeptTask; onOpen: () => void; onPrepare: () => void }) {
  ...
  return <motion.div layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} onClick={onOpen} className={cn("group w-full cursor-pointer rounded-xl border bg-[#ffffff] p-3 text-left transition hover:border-[#0d2b45]/[.15]", task.status === "processing" ? "border-fteal/30" : "border-[#0d2b45]/[.065]")}><div className="mb-2 flex items-center justify-between"><span className="flex items-center gap-1 font-mono text-[7px] uppercase tracking-wider text-fteal"><Bot size={8} /> Team-bot talebi</span><span className="flex items-center gap-1.5">{task.teams && <MessagesSquare size={9} className={task.teams.delivered ? "text-fgreen-light" : "text-[#5B5EA8]"} />}{task.ai ? <span className="flex items-center gap-1 rounded-full border border-fteal/15 px-1.5 py-0.5 text-[8px] text-fteal-light"><Sparkles size={7} /> AI</span> : <span className="text-[8px] text-slate-400">DEMO</span>}</span></div><div className="rounded-lg border border-[#0d2b45]/[.04] bg-[#0d2b45]/[.018] px-2.5 py-2 text-[9px] leading-4 text-slate-500"><span className="text-slate-700">{task.department} ekibi,</span> yeni kampanya için sizden aşağıdaki çalışma bekleniyor.</div><div className="mt-2.5 text-[11px] font-medium leading-[17px] text-slate-800">{task.title}</div><div className="mt-1.5 line-clamp-2 text-[9px] leading-4 text-slate-500">{task.summary}</div>{task.details?.dataSources?.length ? <div className="mt-2 text-[8px] text-slate-400">Talep edilen veri: <span className="text-slate-500">{task.details.dataSources.slice(0,2).join(" · ")}</span></div> : null}<div className="mt-2.5 flex items-center justify-between border-t border-[#0d2b45]/[.05] pt-2">{task.status === "waiting" ? <button onClick={(event) => { event.stopPropagation(); onPrepare(); }} className="flex items-center gap-1 rounded-md border border-fteal/25 bg-fteal/[.07] px-2 py-1 text-[8px] font-semibold text-fteal-light transition hover:bg-fteal/[.13]"><Sparkles size={8} /> AI'ya hazırlat</button> : task.status === "processing" ? <span className="flex items-center gap-1.5 text-[8px] text-fteal-light"><LoaderCircle size={9} className="animate-spin" /> AI taslak hazırlıyor…</span> : <span className="text-[8px]" style={{ color: dept.color }}>{task.status === "done" ? "Tamamlandı" : task.draft ? "Departmanda · AI taslağı hazır" : "Departmanda"}</span>}<ChevronRight size={10} className="text-slate-400 transition group-hover:text-fteal-light" /></div>{task.status === "processing" && <div className="mt-2 h-0.5 overflow-hidden rounded-full bg-[#0d2b45]/[.05]"><motion.div className="h-full w-1/3 rounded-full bg-gradient-to-r from-transparent via-fteal to-transparent" animate={{ x: ["-100%", "300%"] }} transition={{ duration: 1.2, repeat: Infinity, ease: "linear" }} /></div>}</motion.div>;
}

function TaskDrawer({ task, onClose, onAdvance, onPrepare, onSendTeams, sendingTeams }: { task: DeptTask; onClose: () => void; onAdvance: () => void; onPrepare: () => void; onSendTeams: () => void; sendingTeams: boolean }) {
  const dept = DEPT[task.department];
  return <><button onClick={onClose} className="fixed inset-0 z-40 bg-[#10304f]/75 backdrop-blur-sm" /><motion.aside initial={{ x: 500 }} animate={{ x: 0 }} exit={{ x: 500 }} className="fixed bottom-0 right-0 top-0 z-50 w-[470px] overflow-y-auto border-l border-[#0d2b45]/10 bg-[#ffffff]/[.98] p-6"><div className="mb-6 flex items-center justify-between"><div className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[.18em] text-fteal-light"><img src="/rabbit-face.png" alt="Team-bot" className="h-6 w-6 rounded-lg border border-fteal/25 object-cover" /> Team-bot departman paketi</div><button onClick={onClose} className="grid h-8 w-8 place-items-center rounded-lg border border-[#0d2b45]/10 text-slate-600"><X size={14} /></button></div><div className="mb-3 inline-flex rounded-md px-2 py-1 text-[9px] font-bold uppercase" style={{ color: dept.color, background: dept.soft }}>{task.department}</div><div className="mb-4 rounded-xl border border-fteal/12 bg-fteal/[.035] p-3 text-[11px] leading-5 text-slate-600"><span className="font-semibold text-slate-800">Merhaba {task.department} ekibi.</span><br />Onaylanan kampanya için aşağıdaki ihtiyaç ve geliştirme paketi size yönlendirildi.</div><h2 className="text-xl font-semibold leading-7 text-[#0d2b45]">{task.title}</h2><p className="mt-2 text-sm leading-6 text-slate-600">{task.summary}</p>
    ...
    <div className="mt-5 rounded-2xl border border-[#0d2b45]/[.07] bg-[#0d2b45]/[.025] p-4"><div className="mb-2 flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-slate-600"><BrainCircuit size={12} /> Karar gerekçesi</div><p className="text-[13px] leading-6 text-slate-800">{task.rationale}</p></div>
    ...
    {task.details && <><div className="mt-6"><div className="mb-3 flex items-center gap-2 text-[10px] font-semibold uppercase tracking-wider text-slate-600"><Database size={12} /> Kullanılan kaynaklar</div><div className="flex flex-wrap gap-2">{task.details.dataSources.map((source) => <span key={source} className="rounded-full border border-[#0d2b45]/[.08] bg-[#0d2b45]/[.025] px-2.5 py-1 text-[9px] text-slate-600">{source}</span>)}</div></div><div className="mt-6"><div className="mb-3 flex items-center gap-2 text-[10px] font-semibold uppercase tracking-wider text-slate-600"><Layers3 size={12} /> Alt görevler</div><div className="space-y-2">{task.details.subtasks.map((subtask) => <div key={subtask.name} className="rounded-xl border border-[#0d2b45]/[.06] p-3"><div className="flex justify-between gap-3 text-[11px]"><span className="text-slate-800">{subtask.name}</span><span className="shrink-0 font-mono text-[9px] text-fteal-light">{subtask.eta}</span></div><div className="mt-1.5 text-[9px] text-slate-500">{subtask.owner} · {subtask.status}</div></div>)}</div></div><div className="mt-6 rounded-2xl border border-amber/20 bg-amber/[.04] p-4"><div className="mb-2 flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-amber"><Scale size={12} /> Risk & insan denetimi</div><p className="text-xs leading-5 text-slate-600">{task.details.risk}</p></div></>}
    ...
    {task.teams ? (
      <div className={cn("mt-2.5 rounded-xl border p-3", task.teams.delivered ? "border-fgreen/25 bg-fgreen/[.05]" : "border-[#6264A7]/30 bg-[#6264A7]/[.08]")}>
        ...
        <button onClick={onSendTeams} disabled={sendingTeams} className="mt-2 flex items-center gap-1.5 rounded-md border border-[#0d2b45]/[.1] px-2 py-1 text-[9px] text-slate-600 transition hover:text-slate-800 disabled:opacity-40">{sendingTeams ? <LoaderCircle size={10} className="animate-spin" /> : <MessagesSquare size={10} />} Tekrar gönder</button>
      </div>
    ) : (
...
export function TaskBoard({ tasks, campaigns, onAdvance, onUpdateTask, onUpdateCampaign, onCompleteCampaign, onNewCampaign }: {
  ...
  return (
    <div className="mission-bg min-h-screen px-6 py-6 xl:px-8">
      ...
      <AnimatePresence>{selectedTask && <TaskDrawer task={selectedTask} onClose={() => setSelectedTaskId(null)} onAdvance={() => { onAdvance(selectedTask.id); setSelectedTaskId(null); }} onPrepare={() => prepareTask(selectedTask)} onSendTeams={() => sendToTeams(selectedTask)} sendingTeams={sendingTeamsId === selectedTask.id} />}</AnimatePresence>
